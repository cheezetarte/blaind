...
const QnAList = props => {
  ...
  useEffect(() => {
    ...
    // 마지막에 반드시 슬래쉬를 넣어줘야함
    axios.get("http://127.0.0.1:8000/qna/").then((response) => {
  ...
  return (
    <div>
      <CommonTable headersName={['No.', '제목','작성자', '등록일', '조회수']}>
      {text.map((e) => (
        <CommonTableColumn>
          ...
          <CommonTableRow><Link to={`${e.post_id}`} style = {{color :'white'}}>{e.writer}</Link></CommonTableRow>
